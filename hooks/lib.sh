...
guardian_resume_hint_text() {
    printf '[Guardian] Resume: `%s/guardian once` (or `touch %s/proceed_once`); `%s/guardian snooze 15`; `%s/guardian zeno bump` to relax limits toward full usage; or write a future ISO timestamp into %s/snooze_until.' \
        "$GUARDIAN_DIR" "$GUARDIAN_DIR" "$GUARDIAN_DIR" "$GUARDIAN_DIR" "$GUARDIAN_DIR"
}
